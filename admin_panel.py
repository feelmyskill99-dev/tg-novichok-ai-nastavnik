...
# --- HTML sanitize: allowlist b/i/code/a -----------------------------------
_ALLOWED_TAGS = {"b", "strong", "i", "em", "code", "pre", "u", "s"}
...
def sanitize_telegram_html(raw: str) -> str:
    ...
    for m in _TAG_RE.finditer(raw):
        out.append(html_lib.escape(raw[pos:m.start()], quote=False))
        ...
        if tag == "a":
            ...
            out.append(html_lib.escape(m.group(0), quote=False))
        pos = m.end()
    out.append(html_lib.escape(raw[pos:], quote=False))
    return "".join(out)
